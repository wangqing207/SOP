...
if (! length(args==5)){
 print("usage:   /data4/sniu/programs/R-3.0.1/bin/Rscript  ../Rscript/CGI_genetic_info_go_zjl.R  /data5/zhangjl/Project/BC150047-1-chip/diffBind/myc_vs_IgG/ /data4/zhangjl/refData/hg19/hg19_refGene_uscs.bed  9606 genetic /data4/sniu/database/NCBI_gene/  ")
q()
}
...
#now<-getwd()
#setwd("/data4/sniu/database/NCBI_gene/")
...
#filter_macs_refgene<-(macs_refgene_info_tb2[c("chr","start","end","edgeR.logFC","edgeR.logCPM","edgeR.p.value","edgeR.adj.p.value","GeneID","symbol")])
#filter_macs_refgene<-unique(filter_macs_refgene)
#filterFC_macs_refgene<-subset(filter_macs_refgene,abs(filter_macs_refgene$edgeR.logFC)>=1)
#noNA_filter_macs_refgene<-na.omit(filterFC_macs_refgene)
#write.table(noNA_filter_macs_refgene,file=gsub(".xls_qc","",gsub("annotated_genetic_","",macs_genetic_fname)),sep="\t",col.names=T,row.names=F)
