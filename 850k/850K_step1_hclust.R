...
###step 2:QC report 
## samples
a = qcReport(exp, sampNames = pd$Sample_Name, sampGroups = pd$Sample_Group, pdf = "1_Data_QC/QCReport.pdf",maxSamplesPerPage = 40)
###########
pdf("1_Data_QC/Densityplot.pdf")
par(cex=0.65)
c = densityPlot(exp,sampGroups = pd$Sample_Name,xlab = "Beta",pal = colorRampPalette(c("blue","green", "orange", "red"))(40))
print(c)
dev.off()
############
## QC-probesets
...
### beta boxplot 
pdf("2_Preprocess_data/boxplot.pdf")#,width=12,height=12)
...
labelColors <-colorRampPalette(c("blue","orange","red","green"))(4)
g1=pd[pd$Sample_Group=="g1",1]				#为聚类的样本颜色分组
g2=pd[pd$Sample_Group=="g2",1]
g3=pd[pd$Sample_Group=="g3",1]
g4=pd[pd$Sample_Group=="g4",1]
#g3=c("GBL_T2_A","GBL_T2_B")
...
colLab <- function(n) {
    ...
    if (is.leaf(n)) {
	  ...
	  if(a$label %in% g1){i=1}
	  if(a$label %in% g2){i=2}
	   if(a$label %in% g3){i=3}
	  if(a$label %in% g4){i=4}
      labCol <- labelColors[i]
	  print(labCol)
      attr(n, "nodePar") <- c(a$nodePar, lab.col=labCol,branchlength=3)
	  
    }
    n
  }

clusDendro <- dendrapply(as.dendrogram(fit), colLab)  

pdf("2_Preprocess_data/samplecluster.pdf")
a = plot(clusDendro, main = "sample cluster",xlab = "sample",cex = 1)
print(a)
dev.off()
####
